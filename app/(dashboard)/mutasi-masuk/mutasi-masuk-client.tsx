...
export default function MutasiMasukClient() {
  ...
  const [form, setForm] = useState({ tanggal: '', nama: '', nisn: '', nik: '', jenis_kelamin: 'laki-laki', kelas_kelompok: '', sekolah_asal: '', alasan: '', keterangan: '' })
  ...
  const fetchItems = useCallback(async (p: number = 1) => {
    ...
    try {
      ...
      const result = await safeFetch<any>(`/api/kesiswaan/mutasi-masuk?${params}`)
  ...
  const handleAdd = async () => {
    ...
    setSaving(true)
    try {
      await safeFetch('/api/kesiswaan/mutasi-masuk', { method: 'POST', body: JSON.stringify(form) })
      setShowForm(false)
      setForm({ tanggal: '', nama: '', nisn: '', nik: '', jenis_kelamin: 'laki-laki', kelas_kelompok: '', sekolah_asal: '', alasan: '', keterangan: '' })
      fetchItems(1)
  ...
  const handleDelete = async (id: string, nama: string) => {
    if (!confirm(`Yakin hapus mutasi masuk "${nama}"?`)) return
    try {
      await safeFetch(`/api/kesiswaan/mutasi-masuk/${id}`, { method: 'DELETE' })
      fetchItems(page)
    ...
    <div className="page-container">
      <div className="page-header">
        <div>
          ...
          <p className="page-subtitle">Data siswa pindahan masuk</p>
        </div>
      ...
      {showForm && (
        <div className="card p-4 mb-6">
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-sm">
            ...
            <div><label className="block text-xs text-slate-500 mb-1">Sekolah Asal</label><input value={form.sekolah_asal} onChange={e => setForm(f => ({ ...f, sekolah_asal: e.target.value }))} className="input text-sm" /></div>
      ...
      {loading ? (
        ...
        <div className="card p-12 text-center text-slate-400 text-sm">Tidak ada data mutasi masuk</div>
      ) : (
              ...
              <tbody>
                {items.map((r: any) => (
                  <tr key={r.id}>
                    ...
                    <td className="text-sm text-slate-600">{r.sekolah_asal || '-'}</td>
