...
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (row[0].jenis !== 'masuk') return NextResponse.json({ error: 'Not found' }, { status: 404 })
...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!existing.length || existing[0].jenis !== 'masuk') return NextResponse.json({ error: 'Not found' }, { status: 404 })

  const body = await req.json()
  const allowed = ['tanggal','nama','nisn','nik','jenis_kelamin','kelas_kelompok','sekolah_asal','alasan','dokumen_url','keterangan']
