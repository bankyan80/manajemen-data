...
export async function GET(req: NextRequest) {
  ...
  let whereConditions = sql`${studentMutations.jenis} = 'masuk'`
...
export async function POST(req: NextRequest) {
  ...
  if (!body.kelas_kelompok) return NextResponse.json({ error: 'Kelompok tujuan wajib' }, { status: 400 })
  ...
  await db.insert(studentMutations).values({
    ...
    sekolah_asal: body.sekolah_asal || null,
  ...
  const school = await db.select({ jenjang: schools.jenjang }).from(schools).where(eq(schools.id, schoolId)).limit(1)
  if (school.length > 0) {
    await db.insert(students).values({
      school_id: schoolId,
      tahun_pelajaran: '2026/2027',
      jenjang: school[0].jenjang,
      kelas_kelompok: body.kelas_kelompok,
      nama: body.nama,
      nik: body.nik || null,
      nisn: body.nisn || null,
