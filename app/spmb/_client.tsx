              ...
              {ruData?.summary && (
                <div className="grid grid-cols-3 gap-3 p-4">
                  {[
                    ...
                    { key: '_6_7', label: '6ΓÇô7 Tahun', color: 'text-green-700' },
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-sm">
                    <thead><tr className="bg-zinc-50 border-b border-border">
                      ...
                      <th className="text-center px-4 py-3 font-semibold text-text-muted">6ΓÇô7 Thn</th>
      ...
      <div className="card">
        ...
        {showForm && (
          <div className="p-4 border-b border-border bg-zinc-50">
            {nikFound && (
              <div className="mb-3 px-3 py-2 bg-green-50 text-green-700 text-xs rounded-lg font-medium">Data ditemukan ΓÇö form terisi otomatis</div>
            )}
...
function UploadModal({ row, onClose, onUpload }: { row: any; onClose: () => void; onUpload: () => void }) {
  ...
  return (
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal-content max-w-md" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between px-6 py-4 border-b border-border">
          <h3 className="font-semibold text-text-main">Upload Dokumen ΓÇö {row.nama_lengkap}</h3>
          <button onClick={onClose} className="text-text-muted hover:text-text-main text-xl leading-none">&times;</button>
        </div>
        <div className="px-6 py-4 space-y-4 text-sm">
          {[
